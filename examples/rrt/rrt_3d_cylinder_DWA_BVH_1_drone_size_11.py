...
# Speed calculation and adjustment function
def calculate_required_speed(distance, time):
    if time > 0:
        return distance / time
    return float('inf')  # Infinite speed if no time constraint

def adjust_speed_to_reach_goal(path, total_time, dwa_params, label, ax=None):
    if path is None or len(path) == 0:
        print(f"Warning: No path found for {label}, skipping speed adjustment.")
        return

    path_length_value = path_length(path)
    required_speed = calculate_required_speed(path_length_value, total_time)

    # Clip speed to the maximum allowable speed
    adjusted_speed = min(required_speed, dwa_params['max_speed'])
    dwa_params['max_speed'] = adjusted_speed

    # Print speed in terminal
    print(f"Adjusted speed for {label}: {adjusted_speed:.2f} units/s (Distance: {path_length_value:.2f} units, Time: {total_time} s)")

    # Annotate speed on the plot
    if ax is not None:
        midpoint = np.mean(path, axis=0)
        ax.text(midpoint[0], midpoint[1], midpoint[2], f"Speed: {adjusted_speed:.2f} u/s", color='black', fontsize=10)
